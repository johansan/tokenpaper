...
## Configurable Background Export
To generate a background image from a JSON config, run:

`bash scripts/generate_tiled_background.sh`

This reads `config/tiled_background.json` and writes the rendered image to `output/tiled_background.png`.

You can change the following settings in the config file:
- `palette` and `tile_colors` for the tile fills
- `background_color` and `stroke_color` for the canvas and outlines
- `image_width`, `image_height`, `scalar`, and `seed` for framing and density

To use a different config file:

`bash scripts/generate_tiled_background.sh path/to/your-config.json`
